...
const defaultSettings = (): SettingsPayload => ({
  ...
  capital_limit: null as number | null,
...
export const useAppStore = defineStore('app', () => {
  ...
  function applySettings(payload: AppSettings) {
    ...
    settings.capital_limit = payload.capital_limit ?? null
  ...
  async function saveSettings() {
    ...
    try {
      const payload = await api.updateSettings({
        ...
        capital_limit:
          settings.capital_limit === null ||
          settings.capital_limit === undefined ||
          Number.isNaN(Number(settings.capital_limit)) ||
          Number(settings.capital_limit) <= 0
            ? null
            : Number(settings.capital_limit),
      })
